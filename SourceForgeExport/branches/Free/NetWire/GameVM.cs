    ...
    public class GameVM
    {
        ...
        public void RecalcAvailability(BoxVM box)
        {
            ...
            if (visible.Count == _w * _h && MainPage.Instance != null)
            {
                ...
                {
                    if (ActualMirror && _totalRotations <= 0 && W>=3)
                    {
                        var res= MessageBox.Show(
                            AppResources.PerfectMirrorSolution,
                            AppResources.Mirror, MessageBoxButton.OKCancel);
                        if (res == MessageBoxResult.OK)
                            LockScreenHelper.Update(MainPage.Instance.ContentPanel);
                    }
                    else
                    {
