...
namespace NetWireUltimate
{
    public class GameVM
    {
        ...
        public bool ActualMirror { get; set; }
        ...
        public void RecalcAvailability(BoxVM box)
        {
            ...
            if (visible.Count == _w * _h && MainPage.Instance != null)
            {
                ...
                {
                        string messageBoxText = "You win! " + StepsText() + ".";
                    
                    var dialog = new MessageDialog(messageBoxText);
                    dialog.Title = "Congratulations!";
                    //dialog.Commands[(int) dialog.DefaultCommandIndex].Label = W<10? "Next Level":"New Game";
                    dialog.ShowAsync();
        ...
        private string StepsText()
        {
            ...
            if (totalRotations <= 0)
                return "And your solution is perfect.";
            return string.Format("And your solution is {0} steps far from perfect", totalRotations);
        }
        ...
        public static void ChangeDimensions(int width)
        {
            ...
            if(MainPage.Instance==null)
            {
                height = (int)Math.Round(width * 5.0 / 8);
            }
            else
            {
                double actualHeight = Math.Min(MainPage.Instance.ContentPanel.ActualHeight, MainPage.Instance.ContentPanel.ActualWidth);
                double actualWidth = Math.Max(MainPage.Instance.ContentPanel.ActualHeight, MainPage.Instance.ContentPanel.ActualWidth);
                height = (int)Math.Round(width * actualHeight / actualWidth);
            }
            if(height<1||height/width>5)
            {
                height = width;
            }
